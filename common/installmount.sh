...
if [ ! -e /system/etc/init.d/00stop_ric ]; then
		...
		echo "sysctl -w vm.swappiness=60" >> /system/etc/init.d/00stop_ric
		echo "sysctl -w vm.laptop_mode=1" >> /system/etc/init.d/00stop_ric
		echo "sysctl -w vm.drop_caches=1" >> /system/etc/init.d/00stop_ric
